...
i = input("To Load Existing Index Store, enter 1 to load existing store, enter 2 to add weburl, 3 to add filepath, anything else to make new index ")
if (i == "1"):
    db = Chroma(persist_directory=index, embedding_function=embedding)
...
with get_openai_callback() as cb:
    ...
    while True:
            query = input("Query: ")
            topicdata = db.similarity_search(query, k=4)
            #print(topicdata)
